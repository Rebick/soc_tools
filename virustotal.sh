      ...
      if [[ "$undetected" -ne 88 ]]; then
        ...
        if [[ $last_analysis_stats =~ \"malicious\":\ ([1-9][0-9]*),|\"suspicious\":\ ([1-9][0-9]*), ]]; then
          ...
          if [[ "$malicious" -ge 1 || "$suspicious" -ge 1 ]]; then
            ...
            echo -e "${COLOR_GREEN}Se han encontrado coincidencias en${COLOR_RESET} ${COLOR_PURPLE}${ip}${COLOR_RESET}, valores - ${COLOR_ORANGE}Malicious:
${COLOR_RED}${malicious:-0}${COLOR_RESET}, ${COLOR_ORANGE}Suspicious: ${COLOR_RED}${suspicious:-0}${COLOR_RESET}, ${COLOR_ORANGE}Undetected: ${COLOR_RED}${undetected}${COLOR_RESET}"
            encontrado=1
